...
namespace Instinct.TimeX
{
        ...
        where TContext : new()
    {
        ...
        private LinkBase _hibernateLink;
        ...
        protected LinkBase HibernateLink
        {
        ...
        /// <param name="object">The @object.</param>
        /// <param name="time">The time.</param>
        public void Add(LinkBase @object, ulong time)
        {
            //System.Console.WriteLine("Timeline: Add {" + time.ToString() + "}");
            ulong fractionTime = (time & TimePrecision.TimePrecisionMask);
            ulong timeslice = (time >> TimePrecision.TimePrecisionBits);
            if (timeslice < TimesliceArraySize)
            {
                ...
                lock (_addLinkLock)
                {
                    ...
                    {
                        value.Count++;
                        LinkBase chain = value.Chain;
                        @object.NextLink = chain.NextLink;
                        chain.NextLink = @object;
                    }
                }
            }
            else
            {
                ...
                lock (_addLinkLock)
                {
                    ...
                    @object.NextLink = _hibernateLink;
